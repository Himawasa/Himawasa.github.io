...
const REASONS = [
  {
    ...
    w: 1600, h: 1000,
    alt: '名簿の取り込み画面。Excel を置く枠と、同じ人を見分けるキー項目の選択',
  },
  {
    kicker: '02',
    title: '顔写真は、ZIP にまとめて一括登録',
    body: '「社員番号.jpg」のように名簿の値をファイル名にした写真を、ZIP にまとめて置くだけです。名簿と自動照合し、全員分の顔写真をそれぞれのカード枠へ一括配置。結びつかなかった写真の件数もその場で表示されるため、登録漏れを防げます。一人だけ撮り直したときは、名簿の画面からその方の分だけ入れ替えることもできます。',
    img: '/cardsync/screen-photozip.webp',
    w: 1600, h: 622,
    alt: '写真ZIPの取り込み画面。ZIPを置く枠と、紐付ける列の選択',
  },
  {
    kicker: '03',
    title: '顔の位置と大きさは、自動でそろえます',
    body: 'いちばん手間がかかるのが、写真の切り抜きです。CardSync は取り込むときに顔を見つけ、証明写真の決まり（頭の大きさ・目の高さ）に合わせて自動で切りそろえます。スマートフォンで撮ったままの写真で構いません。縦向きで撮った写真の向きも、自動で直します。切るだけで引き伸ばしはしないため、お顔の形が変わることはありません。顔が見つからない写真や、すでに証明写真になっているものは、無理に切らずそのまま取り込みます。',
    img: '/cardsync/photo-trim.webp',
    w: 1200, h: 720,
    alt: 'スマートフォンで撮った写真が、取り込むと証明写真の形に切りそろえられる様子',
  },
  {
    kicker: '04',
    title: '見たまま置ける、カードのデザイン',
    body: '文字・写真・バーコード（CODE128・JAN-13 など）・QR コードを、画面の上で置いて動かすだけ。表と裏を別々に作れます。社名や配色、項目の位置も、その場で調整できます。',
          ...
          <div className="cs-feature-list">
            {REASONS.map((f) => (
              <article className="cs-feature" key={f.title}>
                ...
                <img className="cs-feature-img" src={f.img} alt={f.alt} width={f.w} height={f.h} loading="lazy" />
              </article>
            ))}
          </div>
        </div>
          ...
          <div className="cs-verify-grid">
            <div className="cs-verify-text">
              ...
              <ul className="cs-verify-points">
                <li><strong>作り替えられた QR は、その場で分かります。</strong>中身を1文字でも書き換えると「確かめられませんでした」と表示されます。</li>
                <li><strong>退職された方や、紛失したカードは、名簿から削除すれば使えなくなります。</strong>削除した時点で「すでに使われていません」に変わります。有効期限を過ぎたカードも同様です。</li>
                <li><strong>QR コードの中身に、氏名は書いていません。</strong>お名前は、読み取った方が見る確認画面に、そのときの名簿から表示されます。</li>
              </ul>
              <p className="cs-verify-caveat">なお、本物の QR をそのまま複製したカードは、名簿に残っているあいだは「本物です」と表示されます。カードを紛失された場合は、名簿から削除してください。</p>
            </div>
